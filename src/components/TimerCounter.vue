...
<template>
  <div>
    <div class="flex text-6xl font-bold font-mono p-4 bg-gray-200/80 rounded-2xl">
      <span>{{ hour.toString().padStart(2, '0') }}:</span>
      <span>{{ minute.toString().padStart(2, '0') }}:</span>
      <span>{{ second.toString().padStart(2, '0') }}</span>
    </div>
    <div class="flex gap-4 text-gray-200 justify-around mt-4">
      <button v-show="!isPlaying" @click="togglePlay">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-10">
